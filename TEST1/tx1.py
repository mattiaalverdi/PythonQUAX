...
try:
    ...
    while True:
        try:
            ...
            # Salva le stampe su file CSV
            save_path = 'PLC/PYTHON/log1.csv'  # Cambia questo percorso con quello desiderato

            with open(save_path, mode='w', newline='') as file:
                writer = csv.writer(file)
                
                # Scrivi l'ora corrente
                writer.writerow([f"LOG of {current_time()}"])
                
                # Decodifica degli stati delle pompe
                for i in range(5):
                    register_value = pumps_rr.registers[i]
                    bit_8, bit_9, bit_10, bit_11 = check_bits(register_value)
                    writer.writerow([f"Pump {PumpsHeader[i]}"])
                    writer.writerow([f"  Start: {'High' if bit_8 else 'Low'}"])
                    writer.writerow([f"  OK: {'High' if bit_9 else 'Low'}"])
                    writer.writerow([f"  Warning: {'High' if bit_10 else 'Low'}"])
                    writer.writerow([f"  Alarm: {'High' if bit_11 else 'Low'}"])

                # Decodifica delle letture dei CH del CCi
                writer.writerow([f"CryCon 18i"])
                for idx, value in enumerate(CCi_channels):
                    writer.writerow([f"{CCiHeader[idx]}: {round_to_significant_figures(value, 4)} K"])

                # Decodifica delle letture delle gauges della PMG
                writer.writerow([f"Pfeiffer MaxiGauge"])
                float_values = registers_to_float(PMG_gauges_rr.registers)
                for i in range(6):
                    writer.writerow([f"{PMGHeader[i]}: {round_to_significant_figures(float_values[i], 4)} mbar"])

                # Decodifica delle letture delle gauges della PSG
                writer.writerow([f"Pfeiffer SingleGauge"])
                float_values = registers_to_float(PSG_gauges_rr.registers)
                for i in range(2):
                    writer.writerow([f"{PSGHeader[i]}: {round_to_significant_figures(float_values[i], 4)} mbar"])

                # Decodifica delle letture dell'HDI
                writer.writerow([f"Helium Depth Indicator"])
                float_values = registers_to_float(HDI_read_rr.registers)
                writer.writerow([f"Channel: {HDI_ch}"])
                writer.writerow([f"Read: {round_to_significant_figures(float_values[0], 4)} {HDI_unit}"])
                writer.writerow([f"Channel: {HDI_state}"])
